...
class User extends CI_Controller
{
	...
	public function adminhome()
	{
		...
		if ($this->session->userdata('User')) {
			$usrinfo = new users_model;
			$data['data'] = $usrinfo->get_requestEmer();
			$datamodal['datamodal'] = $usrinfo->get_responder();
			// var_dump($datamodal[0]->userInfo_ID);
			$this->load->view('includes/header', $datamodal);
	...
	public function responderhome()
	{
		...
		if ($this->session->userdata('User')) {
			...
			$data['data'] = $usrinfo->getresponderfromForward($Responder_ID);
			$this->load->view('includes/resheader', $Responder_ID);
			$this->load->view('responder_view', $data);
