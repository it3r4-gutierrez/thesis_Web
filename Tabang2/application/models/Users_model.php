...
class Users_model extends CI_Model
{
    ...
    {
        $query = $this->db->query('SELECT tbl_emergencyreport.Ereport_ID,tbl_emergencyType.EmegencyType,tbl_Resident.*,tbl_emergencyreport.date,
        ...
        ON tbl_emergencyreport.resident_ID = tbl_Resident.resident_ID INNER JOIN tbl_emergencyType ON tbl_emergencyType.EtypeID = tbl_emergencyreport.EtypeID 
         WHERE tbl_emergencyreport.status_id = 0');
        return $query->result();
    }
    public function get_requestEmerForwarded()
    {
        ...
        WHERE tbl_emergencyreport.status_id = 1');
        return $query->result();
    }
    ...
    public function CreateRes($responderDatauserinfo, $responderData)
    {
        if ($this->db->insert('tbl_userinfo', $responderDatauserinfo)) {
            $query1 = $this->db
                ->select('userInfo_ID')
                ->where('username', $responderDatauserinfo['username'])
                ->get('tbl_userinfo');
    ...
    public function CreateResident($residentDatauserinfo, $residentData)
    {
        if ($this->db->insert('tbl_userinfo', $residentDatauserinfo)) {
            $query1 = $this->db
                ->select('userInfo_ID')
                ->where('username', $residentDatauserinfo['username'])
                ->get('tbl_userinfo');
            $res = $query1->row_array();
    ...
    {
        $query = $this->db->query('SELECT tbl_emergencyType.EmegencyType,tbl_Resident.ContactNumber,tbl_ForwardReport.Status_incident,tbl_ForwardReport.Freport_ID,
        ...
        tbl_emergencyType.EtypeID = tbl_emergencyreport.EtypeID
         ...
         tbl_ForwardReport.Status_incident = "Pending" OR tbl_ForwardReport.Status_incident = "OnGoing"');
        return $query->result();
    }
    public function getresponderfromForwardDONE($Responder_ID)
    {
        ...
        tbl_Responder.Responder_ID = tbl_ForwardReport.Responder_ID LEFT JOIN tbl_Resident ON tbl_Resident.resident_ID = tbl_emergencyreport.resident_ID
         WHERE tbl_emergencyreport.status_id = 1 AND tbl_ForwardReport.Status_incident = "Done" AND tbl_Responder.Responder_ID =' . $Responder_ID);
        return $query->result();
    }
}